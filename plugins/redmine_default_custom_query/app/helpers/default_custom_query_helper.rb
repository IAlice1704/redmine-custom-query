module DefaultCustomQueryHelper
  def options_for_selectable_queries(project)
    ...
    if queries.any?
      options_groups << [l('default_url_options.label_queries_for_all_projects'), queries]
    end

    queries = project.queries.only_public
    if queries.any?
      options_groups << [l('default_url_options.label_queries_for_current_project'), queries]
    end
