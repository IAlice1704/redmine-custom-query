...
class ManageDefaultQueryTest < Redmine::IntegrationTest
  def setup
    @user = create(:user)
    @project = create(:project, :with_default_url_options)
    logged_in @user
  end
  ...
  def test_logged_user_has_not_manage_default_query_permission
    ...
    assert_select '#tab-default_url_options', false

    # update
    put default_url_options_setting_update_path(@project)
  ...
  def test_logged_user_has_manage_default_query_permission
    ...
    get settings_project_path(@project, tab: 'default_url_options')

    # Render
    assert_response :success
    assert_template partial: 'default_url_options_setting/_form'
    ...
    assert_difference -> { @project.default_queries.count }, 1 do
      xhr :put, default_url_options_setting_update_path(@project),
